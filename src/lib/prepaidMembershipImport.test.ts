...
import { normalizeImportedDate, parseCsvText, parsePrepaidMembershipCsv } from "./prepaidMembershipImport";
...
describe("prepaid membership CSV import", () => {
  ...
  it("keeps rows with incomplete membership data but reports review errors", () => {
    ...
    expect(result.errors[0]).toEqual({
      row: 2,
      message:
        "Dati tesseramento incompleti: birth_date, birth_place, province_of_birth, residential_address, city_of_residence, province_of_residence.",
    });
  });
});
